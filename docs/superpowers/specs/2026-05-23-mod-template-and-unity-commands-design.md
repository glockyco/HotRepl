...
**Status:** Draft for review — incorporates v0.2 corrections from research-driven self-review
(§12).\
...
1. **A new strongly-typed command interface in `HotRepl.Core`**
   ...
   C# arg/result POCO types via a small reflection-based "hand-write a `JObject` schema literal next
   to every command, hope it matches your deserialization" footgun that every existing consumer has
   hit.
...
Schemas are produced by a **hand-rolled minimal generator in `HotRepl.Core`** that reflects over
public properties of a POCO and emits the JSON Schema as a `JObject`. Cached per `Type` in a
thread-safe dictionary; pre-warmed at adapter construction (plugin Awake), not on first request.

**Why hand-rolled, not NJsonSchema (the obvious off-the-shelf choice):** NJsonSchema 11.x declares a
transitive `System.Text.Json (>= 9.0.9)` dependency. Even the `NJsonSchema.NewtonsoftJson` variant
pulls `System.Text.Json` through that chain. Mono BepInEx games have a documented history of
`System.Text.Json` assembly-loading failures; shipping STJ + its dependency tree into every
consumer's plugin folder is real, avoidable deployment risk. Newtonsoft.Json.Schema is dual-licensed
under commercial + AGPL — incompatible with HotRepl's MIT license. NJsonSchema 10.x is unmaintained.
A ~200-LOC hand-rolled generator that covers exactly what typed HotRepl commands need is the
defensible long-term choice: no third-party blast radius, no library churn to track, no licensing
concern, exact control over emitted schema shape.
...
| `T[]`, `List<T>`, `IEnumerable<T>` | `{ "type": "array", "items": <T> }`                                       |
| `Dictionary<string, T>`            | `{ "type": "object", "additionalProperties": <T> }`                       |
| enum                               | `{ "type": "string", "enum": [...] }` (always serialized as string)       |
| nested POCO                        | `{ "type": "object", "properties": { ... }, "required": [...] }`          |
| `EmptyArgs`                        | `{ "type": "object", "additionalProperties": false }` (special case)      |
| anything else                      | falls back to `SchemaCache.AnyObject` with a warning logged once per type |

**Attributes honored:**

| Attribute                                                   | Where it goes                                                          |
| ----------------------------------------------------------- | ---------------------------------------------------------------------- |
| `[Required]` (from `System.ComponentModel.DataAnnotations`) | adds property name to parent's `required` array                        |
| `[Range(min, max)]`                                         | adds `minimum`/`maximum` to number schemas                             |
| `[Description("…")]` (from `System.ComponentModel`)         | adds to `description` field                                            |
| `[JsonProperty("wireName")]` (Newtonsoft)                   | renames the property in the schema to match the wire-format serializer |

These four cover every attribute the four demo commands use and every attribute Ardenfall's migrated
commands will use. If a future command needs something we don't support yet, the fallback is
well-defined (`AnyObject` + log warning) and the handler can override by passing an explicit
`argsSchema` / `resultSchema` JObject through a future overload — out of scope for v1.

**Generator structure:**

```csharp
// HotRepl.Core/Schema/SchemaCache.cs — public so consumers can hand-warm.
public static class SchemaCache {
    ...
    /// used as the universal artifacts-schema fallback.</summary>
    ...
    public static JObject For(Type t) =>
        _cache.GetOrAdd(t, static type => SchemaBuilder.Build(type));
}

// HotRepl.Core/Schema/SchemaBuilder.cs — internal, the reflection logic.
internal static class SchemaBuilder {
    public static JObject Build(Type t) {
        // ~150-200 LOC: handle primitives, nullables, collection types, dictionaries,
        // enums, nested objects, EmptyArgs, fallback. Read attributes via
        // member.GetCustomAttribute<…>().
    }
}
```

**Why `JObject` on the wire:** the existing `ControlCommandDescriptor` already uses `JObject` for
`argsSchema`/`resultSchema`. `JObject` is the boundary; the schema generator is a
`HotRepl.Core`-internal implementation detail. Consumers never see the generator type and can't
accidentally couple themselves to its surface.

**Why pre-cache at registration:** the first reflection-driven Build per type takes a few ms. Doing
this at plugin Awake — when all eyes are already on "the game is loading" — instead of on the first
agent call removes a jitter source from the agent experience.

### 2.4 Zero new third-party dependencies on `HotRepl.Core`

Critical consequence of §2.3's hand-rolled approach: **no NJsonSchema, no Namotion.Reflection, no
System.Text.Json**. `HotRepl.Core`'s public surface gains the typed interface and the schema cache;
its private surface gains the schema builder. Its dependency set stays exactly what it is today
(Newtonsoft.Json + Fleck transitively via HotRepl.Protocol). Deployment remains a single bundle of
HotRepl DLLs side-by-side in the game's plugin folder. Every existing consumer deploy script keeps
working without file-list changes.

The ~200 LOC of schema generator code is a maintenance cost we accept as the price of zero
third-party blast radius. The alternative — shipping NJsonSchema's dependency chain into every
consumer's game — has higher expected cost (debugging Mono assembly-load failures across the
consumer ecosystem over time) than the upfront engineering cost.
